...
func (a *app) create(args []string) error {
	...
	dst, err := a.createRepo(fs.Arg(0), *ssh)
	if err != nil {
		return err
	}
	_ = repo.Bump(dst)
...
// createRepo makes an empty repository where ref says it belongs and returns
// its path. The finder's /create goes through here too, so the two cannot
// drift apart.
func (a *app) createRepo(ref string, ssh bool) (string, error) {
